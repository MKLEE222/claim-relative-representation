...
def note1_aliases(text,seed_pos):
    ...
    heading_end=seed_pos+m.end()
    tail=text[heading_end:]
    tail=re.sub(r"^[\s—–-]+","",tail)

    # First prose sentence after the heading. The D1 bug incorrectly started
    # sentence termination at the period inside "1." in the heading.
    sm=re.search(r"(.+?\.)",tail,flags=re.S)
    if not sm:
        raise RuntimeError("Note 1 first prose sentence not found")
    sent=" ".join(sm.group(1).split())
    ...
    # Also admit title-case hyphenated / city-form aliases from the identity
    # sentence if they are explicitly present there.
    for tok in re.findall(r"\b[A-Z][a-z]+(?:-[A-Za-z]+)+\b",sent):
        expanded.append(tok.lower())
        expanded.extend(p.lower() for p in tok.split("-") if len(p)>=3)

    seen=set()
    aliases=[]
    for x in expanded:
        if x not in seen:
            seen.add(x); aliases.append(x)
    return aliases,sent
...
seed,seed_pos=locate_seed(text)
...
overlap=set(t for t,w in bridged) & FORBIDDEN
if overlap:
    raise RuntimeError(f"target-only leakage detected: {sorted(overlap)}")
...
print("EP_KS_01D2_NATIVE_REFERENT_BRIDGE_REPAIR")
print("source_sha256="+sha)
print("identity_sentence="+sentence)
...
print("D1_STATUS=INVALID_IMPLEMENTATION")
